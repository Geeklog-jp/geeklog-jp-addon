...
if ($op == "last10posts") {
    ...
    $emaillink    = '<a href="%s"><img src="'.$CONF_FORUM['imgset'].'/email.gif" TITLE="'.$LANG_GF01['EmailLink'].'"></a>';
    $pmlink       = '<a href="%s"><img src="'.$CONF_FORUM['imgset'].'/profile.gif" TITLE="'.$LANG_GF01['PMLink'].'"></a>';
    $homepagelink = '<a href="%s"><img src="'.$CONF_FORUM['imgset'].'/home.gif" TITLE="'.$LANG_GF01['WebsiteLink'].'"></a>';
    ...
    while($siteMembers = DB_fetchArray($query)) {
        ...
        if ($siteMembers['posts'] > 0) {
            ...
            $report->set_var ('image', 'latestposts.'.$_IMAGE_TYPE);            
        ...
        if($siteMembers['emailfromuser'] == '1') {
            ...
            $report->set_var ('image', 'email.'.$_IMAGE_TYPE);             
        ...
        if ($CONF_FORUM['use_pm_plugin']) {
            ...
            if ($pmplugin_link != '') {
                $report->set_var ('image', 'pm.'.$_IMAGE_TYPE);                 
        ...
        if($siteMembers['homepage'] != '') {
            ...
            $report->set_var ('image', 'home.'.$_IMAGE_TYPE);              
        ...
        if($siteMembers["homepage"] != '') {
            $memberhomepage = '<A HREF="' .$siteMembers['homepage']. '" TARGET="opennewwindow">' .$siteMembers['homepage']. '</A>';
        } else {
            $memberhomepage = $LANG_GF01['NA'];
        }
        ...
        $report->set_var ('member_homepage', $memberhomepage);
