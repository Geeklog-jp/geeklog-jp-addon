...
require_once ($_CONF['path'] . 'plugins/nexfile/functions.inc');
...
$FEATURES = array();
$FEATURES['nexfile.admin']       = 'Administrative access to ' . $pi_display_name;
$FEATURES['nexfile.user']        = "Plugin user permission - Required if user will have edit rights to pages";
$FEATURES['nexfile.edit']        = "Plugin Selected File and Category Admin";

$MAPPINGS = array();
$MAPPINGS['nexfile.admin']         = array ($pi_admin);
$MAPPINGS['nexfile.user']          = array ($pi_admin);
$MAPPINGS['nexfile.edit']          = array ($pi_admin);
...
function show_prerequisites() {
    ...
    $fp = @fopen($_CONF['path_html'] . $pi_name . '/data/test.txt', 'w');
    if ($fp != NULL) {
        fclose($fp);
        unlink($_CONF['path_html'] . $pi_name . '/data/test.txt');
    ...
    if ($image == 'icon_fail.png') {
        $disabled = ' disabled="disabled"';
        COM_errorLog("Cannot install $pi_name: Cannot write to \"{$_CONF['path_html']}{$pi_name}/data/\"");
    }
    $p->set_var('prereqs', '<li style="list-style-image: url('.$_CONF['layout_url'].'/'.$pi_name.'/images/admin/'.$image.');"> Write permissions on "'.$_CONF['path_html'].$pi_name.'/data/"', true);
