...
function plugin_initconfig_{plugin}()
{
    ...
    if (!$c->group_exists($me)) {
        ...
        $c->add('{plugin}loginrequired', $_{conf_var_prefix}_DEFAULT['{plugin}loginrequired'], 'select', 0, 0, 0, 10, true, $me);

        $c->add('hide{plugin}menu', $_{conf_var_prefix}_DEFAULT['hide{plugin}menu'], 'select', 0, 0, 0, 30, true, $me);

        $c->add('aftersave', $_{conf_var_prefix}_DEFAULT['aftersave'], 'select', 0, 0, 9, 40, true, $me);

        $c->add('fs_permissions', NULL, 'fieldset', 0, 2, NULL, 0, true, $me);

        $c->add('default_permissions', $_{conf_var_prefix}_DEFAULT['default_permissions'], '@select', 0, 2, 12, 140, true, $me);
    }
