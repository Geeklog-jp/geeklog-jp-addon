...
require_once ($_CONF['path'] . 'plugins/nexflow/functions.inc');

// Plugin information
$pi_display_name = 'nexflow';
$pi_name         = 'nexflow';
...
$FEATURES = array();
$FEATURES['nexflow.admin']       = 'Administrative access to ' . $pi_display_name;
$FEATURES['nexflow.user']        = "nexFlow Access";
$FEATURES['nexflow.edit']        = "nexFlow template, variable and handler editing";

$MAPPINGS = array();
$MAPPINGS['nexflow.admin']         = array ($pi_admin);
$MAPPINGS['nexflow.user']          = array ($pi_admin);
$MAPPINGS['nexflow.edit']          = array ($pi_admin);
...
function show_prerequisites() {
    ...
    $p->set_var('prereqs', '<li style="list-style-image: url('.$_CONF['layout_url'].'/'.$pi_name.'/images/admin/'.$image.');"> nexPro is installed and enabled', true);

    //test if nexform plugin is installed
    $image = (DB_getItem($_TABLES['plugins'], 'pi_name', "pi_name='nexform'") != '') ? 'icon_check.png':'icon_fail.png';
    if ($image == 'icon_fail.png') {
        $disabled = ' disabled="disabled"';
        COM_errorLog("Cannot install $pi_name: nexForm is not installed or enabled");
    }
    $p->set_var('prereqs', '<li style="list-style-image: url('.$_CONF['layout_url'].'/'.$pi_name.'/images/admin/'.$image.');"> nexForm is installed and enabled', true);
